...
24. Verificación de que nada más cambió: `POST /lotes`, `PATCH /lotes/:id/rechazar`, `PATCH /lotes/:id/aprobar`, los dos `GET /pesajes*` y los dos `GET /clientes*` con sus filtros, `GET /permisos/me` y los tres `GET /catalogos/*` responden igual.
...
- [ ] Los dos `GET /pesajes*` y los dos `GET /clientes*` con sus filtros de SPEC 16 y SPEC 17, los cuatro `PATCH`, `POST /lotes`, `POST /clientes`, `POST /pesajes`, `POST /auth/login`, `POST /auth/register` y los tres `GET /catalogos/*` siguen funcionando igual.
...
- **Sí:** los dos endpoints reciben **los mismos siete filtros**. **Decisión explícita del usuario**, igual que en SPEC 17. La única diferencia entre el par sigue siendo `validateVinculoOperador`.
