...
- **Write path convention:** every create opens `this.db.transaction().execute(async (trx) => ...)`, runs a series of `private async validateX(..., db: Kysely<Database>)` helpers **inside** the transaction (they take the `trx` so validation and insert share a connection), then inserts and returns `Number(result.insertId)`. Validators throw `BadRequestException` for missing/inactive rows and duplicates, `ForbiddenException` for authorization. Copy this shape for new modules. Spec 10's `rechazarPesaje` is the project's only **`UPDATE`** and keeps the same shape (transaction, validators taking the `trx`), returning `true` instead of an id — but see the access-control note above before copying it.
- **Access control convention:** most operations touching a client's data call a `validateVinculoOperador(clienteId, usuarioId, db)`-style check against `cliente_operador` first. For `pesajes` the `cliente_id` is resolved from the lote first. **This is no longer a rule the codebase actually holds to** — four endpoints deliberately skip the check: spec 08's `GET /clientes/all`, spec 09's three `catalogos` reads, and spec 10's `PATCH /pesajes/:id/rechazar`. The last one matters most, because it is a write: `PesajesRepository` now has `createPesaje` validating the link and `rechazarPesaje`, a few lines below it, not validating it — the method that guards is the one that creates, not the one that destroys. Do not copy `rechazarPesaje` as the template for a new write. Today the `GET /clientes` filter is cosmetic for anyone who knows the other routes exist.
...
Beyond the `GET`s above and spec 10's `PATCH`, there is no `GET /clientes/:id`, no `GET /lotes/:id`, no `PUT` or `DELETE` anywhere, and no CRUD on the three catalog tables. `PATCH /pesajes/:id/rechazar` is the project's only non-`GET`/`POST` route, its only `UPDATE`, and its only `:id` path param. Deletion is always logical (`isActive = 0`); nothing issues a `DELETE FROM`. If `GET /clientes/:id` is ever added, declare it **after** `@Get('all')` in `ClientesController` or it will swallow `/clientes/all`.

Two routes are mapped by Nest but not covered by any spec: `GET /lotes/cliente/:clienteId/all` (still undocumented — check `LotesController` before assuming what it returns) and `GET /pesajes/byLote/:loteId`, which spec 10 documented after the fact. Trust the route log over this table if the two ever disagree.
...
- Any DDL is applied **by hand in MySQL**; there is no migration tooling and no `.sql` files in the repo. When a change needs schema work, document the DDL in the spec and update `src/database/types/types.ts` to match. (Specs 02 and 03 state "no DDL" as a decision — note that `pesajes.fuera_de_rango` was in fact added during spec 03, so that spec's acceptance criterion is inaccurate.) Spec 10 is the only spec so far that declares its DDL up front: three nullable columns on `pesajes` and a real FK from `rechazado_por` to `usuarios(id)`. That FK is the second exception to the application-code-only rule, after `permisos` → `roles`, justified as an audit column that would be worthless pointing at a missing user. Note the inconsistency it leaves inside one table: `pesajes.rechazado_por` has a FK and `pesajes.usuario_id` still does not.
