...
No read, update or delete endpoints exist beyond the four `GET`s above — no `GET /clientes/:id`, no `GET /lotes/:id`, no `GET /pesajes/*`, no `PUT`/`PATCH`/`DELETE` anywhere. If `GET /clientes/:id` is ever added, declare it **after** `@Get('all')` in `ClientesController` or it will swallow `/clientes/all`.
