...
**Module layout convention** (established by `src/modules/auth`): each feature module has `*.controller.ts` → `*.service.ts` → `repository/*.repository.ts` → `dto/*.dto.ts` (Zod schemas). Controllers stay thin, and services currently just pass through to repositories — but the repository is where business logic actually lives too (e.g. `AuthRepository` does password hashing with `bcrypt` and signs the JWT via `JwtService`, not just Kysely queries), so don't assume repositories are pure data-access when extending this pattern. `src/modules/pesajes` exists as an empty stub for a not-yet-implemented module — follow the auth module's file layout when building it out.
